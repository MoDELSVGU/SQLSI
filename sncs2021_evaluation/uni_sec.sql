...
SELECT res INTO result FROM 
(SELECT 
(EXISTS (SELECT 1 FROM Enrollment WHERE lecturers = kcaller AND kstudents = students)) or (EXISTS (SELECT 1 FROM Enrollment e1 JOIN Enrollment e2 ON e1.students = e2.students WHERE e1.lecturers = kcaller AND e2.lecturers = klecturers))as res
) AS TEMP;
RETURN (result);
END //
DELIMITER ;
