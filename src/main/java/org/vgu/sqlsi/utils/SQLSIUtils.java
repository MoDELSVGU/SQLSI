...
import org.vgu.sqlsi.sql.select.SelectBasicAssociationClass;
...
public class SQLSIUtils {
  ...
  private static SelectBody classifySelectBody(DataModel dataModel, SelectBody selectBody)
      throws Exception {
  ...
  private static SelectSQLSI classifySelectWithSingleJoin(
      DataModel dataModel, FromItem fromItem, List<Join> joins) throws Exception {
    ...
    if (fromItem instanceof SubSelect) {
      ...
      if (dataModel.getAssociations() != null
          && dataModel.getAssociations().stream()
  ...
  private static SelectSQLSI classifySelectWhenLeftItemIsClass(DataModel dataModel, Join join)
      throws Exception {
  ...
  private static SelectSQLSI classifySelectWhenLeftItemIsSubSelect(DataModel dataModel, Join join)
      throws Exception {
    SelectSQLSI selectSQLSI;
    Join newJoin = new Join();
    FromItem rightItem = join.getRightItem();
  ...
  private static SelectSQLSI classifySelectBasic(DataModel dataModel, FromItem fromItem)
      throws Exception {
    ...
    if (fromItem instanceof SubSelect) {
      ...
      } else if (dataModel.getAssociationClasses() != null
          && dataModel.getAssociationClasses().stream()
              .anyMatch(assocClass -> assocClass.getName().equals(table.getName()))) {
  ...
  public static String getAssociationSource(DataModel dataModel, String className, String endName) {
    if (dataModel.getAssociations() == null || dataModel.getAssociations().isEmpty()) {
      ...
      for (Association as : dataModel.getAssociations()) {
        if (as.getLeftEntityName().equals(className) && as.getLeftEnd().equals(endName)) {
          return as.getName();
        } else if (as.getRightEntityName().equals(className) && as.getRightEnd().equals(endName)) {
          return as.getName();
        } else {
          continue;
        }
  ...
  public static AuthFunc findAuthFunctionAssociation(
      DataModel dataModel, List<AuthFunc> functions, String columnName, String associationName)
      throws Exception {
  ...
  public static AuthFunc findAuthFunctionAttribute(
      List<AuthFunc> functions, String columnName, String tableName) throws Exception {
    Optional<AuthFunc> authFun;
    if (functions == null || functions.isEmpty()) {
      throw new Exception("There is no auth function created");
    } else {
