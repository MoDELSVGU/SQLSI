...
import org.vgu.sqlsi.sql.select.SelectSQLSI;
...
public class SQLSIUtils {
    ...
    public static void classify(Statement statement, DataModel dataModel)
        throws Exception {
    ...
    private static void classifySelect(Statement statement, DataModel dataModel)
        throws Exception {
        Select select = (Select) statement;
        SelectBody selectBody = select.getSelectBody();
        if (selectBody != null && selectBody instanceof PlainSelect) {
            SelectBody newSelectBody = classifySelectBody(dataModel,
                selectBody);
    ...
    private static SelectBody classifySelectBody(DataModel dataModel,
        SelectBody selectBody) throws Exception {
        if (selectBody instanceof PlainSelect) {
            ...
            if (joins == null || joins.size() == 0) {
                ...
                FromItem fromItem = plainSelect.getFromItem();
                newSelectBody = classifySelectWithSingleJoin(dataModel,
                    fromItem, joins);
            } else {
                throw new Exception("Not supported type of statement");
            }
    ...
    private static SelectSQLSI classifySelectWithSingleJoin(DataModel dataModel,
        FromItem fromItem, List<Join> joins) throws Exception {
        ...
        if (fromItem instanceof SubSelect) {
            ...
            selectSQLSI = classifySelectWhenLeftItemIsSubSelect(dataModel,
                join);
        } else if (fromItem instanceof Table) {
            Table table = (Table) fromItem;
            newFromItem = table;
            if (dataModel.getAssociations() != null
                && dataModel.getAssociations().stream().anyMatch(
                    assoc -> assoc.getName().equals(table.getName()))) {
                selectSQLSI = classifySelectWhenLeftItemIsAssociation(dataModel,
                    join);
            } else if (dataModel.getEntities() != null
                && dataModel.getEntities().containsKey(table.getName())) {
                selectSQLSI = classifySelectWhenLeftItemIsClass(dataModel,
                    join);
            } else {
                throw new Exception("Not supported type of statement");
            }
    ...
    private static SelectSQLSI classifySelectWhenLeftItemIsClass(
        DataModel dataModel, Join join) throws Exception {
    ...
    private static SelectSQLSI classifySelectWhenLeftItemIsAssociation(
        DataModel dataModel, Join join) throws Exception {
        SelectSQLSI selectSQLSI;
        Join newJoin = new Join();
        FromItem rightItem = join.getRightItem();
    ...
    private static SelectSQLSI classifySelectWhenLeftItemIsSubSelect(
        DataModel dataModel, Join join) throws Exception {
        SelectSQLSI selectSQLSI;
        Join newJoin = new Join();
        FromItem rightItem = join.getRightItem();
    ...
    private static SelectSQLSI classifySelectBasic(DataModel dataModel,
    ...
    private static SubSelect selectSS(DataModel dataModel, SubSelect subSelect)
        throws Exception {
        SubSelect newSubSelect = new SubSelect();
        newSubSelect.setAlias(subSelect.getAlias());
        newSubSelect.setSelectBody(
            classifySelectBody(dataModel, subSelect.getSelectBody()));
        return newSubSelect;
    }
            ...
            if (((JSONObject) entity).containsKey("class")) {
                if (((JSONObject) entity).get("class").equals(className)) {
                    if (((JSONObject) entity).containsKey("attributes")) {
                        attributes = (JSONArray) ((JSONObject) entity)
                            .get("attributes");
                    }
    ...
    public static JSONArray getAssociationsFromClass(JSONArray context,
        String className) {
    ...
    public static Association getAssociation(DataModel dataModel,
        String association) {
        if (dataModel.getAssociations() == null
            || dataModel.getAssociations().isEmpty()) {
            return null;
        } else {
    ...
    public static String getAssociationSource(DataModel dataModel,
        ...
        if (dataModel.getAssociations() == null
            ...
            for (Association as : dataModel.getAssociations()) {
                if (as.getLeftEntityName().equals(className)
                    && as.getLeftEnd().equals(endName)) {
                    return as.getName();
                } else if (as.getRightEntityName().equals(className)
                    && as.getRightEnd().equals(endName)) {
                    return as.getName();
                } else {
                    continue;
                }
    ...
    public static String getEndClassName(DataModel context,
        Association association, String endName) {
        if (association.getLeftEnd().equals(endName)) {
            return association.getLeftEntityName();
        }
        if (association.getRightEnd().equals(endName)) {
            return association.getRightEntityName();
        }
        return null;
    }

    public static boolean isAttribute(DataModel dataModel, String entityName,
        String attribute) {
        if (dataModel.getEntities() == null
            || dataModel.getEntities().isEmpty()) {
            return false;
        }
    ...
    public static boolean isAssociationEnd(DataModel dataModel,
        String className, String endName) {
        if (dataModel.getAssociations() == null
            || dataModel.getAssociations().isEmpty()) {
            return false;
        ...
        for (Association assoc : dataModel.getAssociations()) {
            ...
            if (assoc.getLeftEnd().equalsIgnoreCase(endName)) {
                return true;
            }
            if (assoc.getRightEnd().equalsIgnoreCase(endName)) {
                return true;
            }
        }
        return false;
    ...
    public static boolean isAssociation(JSONArray context, String className,
        String endName) {
    ...
    public static boolean isPrimaryKey(DataModel dataModel, String columnName,
        String tableName) {
    ...
    public static boolean isSuperKey(JSONArray context, String columnName) {
        ...
        for (Object entity : context) {
            if (((JSONObject) entity).containsKey("class")) {
                if (((JSONObject) entity).containsKey("super")) {
                    String superClass = (String) ((JSONObject) entity)
                        .get("super");
    ...
    // super
    public static boolean isSuper(JSONArray context, String entityName,
    ...
    public static boolean isClass(DataModel dataModel, String entityName) {
    ...
    public static AuthFunc findAuthFunctionAssociation(
        ...
        if (functions == null || functions.isEmpty()) {
            ...
            authFun = functions.stream()
                .filter(f -> f.getResource().equals(authFunName)).findFirst();
        }
