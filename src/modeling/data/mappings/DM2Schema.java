...
public class DM2Schema {
	...
	public static void main(String[] args) throws Exception {
		...
		File securityModelFile = new File("src/main/resources/genSQL/uni_pof_sm.json");

		DataModel dataModel = new DataModel(new JSONParser().parse(new FileReader(dataModelFile.getAbsolutePath())));
		FileWriter fileWriter = new FileWriter(SQLschemaFile);

		SecurityModel securityModel = new SecurityModel(
				new JSONParser().parse(new FileReader(securityModelFile.getAbsolutePath())));
		...
		List<Statement> entityStatements = generateEntityStatements(dataModel, securityModel);
		schema.addAll(entityStatements.stream().map(Statement::toString).collect(Collectors.toList()));
