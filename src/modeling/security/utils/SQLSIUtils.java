...
import modeling.security.mappings.SelectJoinAssociationClassAndClass;
import modeling.security.mappings.SelectJoinAssociationClassAndSubSelect;
...
//                    selectSQLSI = SelectJoinAssociationClassAndAssociationClass(dataModel, join);
    ...
    public static boolean isAssociationClassEnd(DataModel dataModel, String tableName, String columnName) {
        if (!isEntity(dataModel, tableName)) {
            return false;
        }
        Entity entity = dataModel.getEntities().get(tableName);
        for (End_AssociationClass end_asc : entity.getEnd_acs()) {
            if (end_asc.getName().equals(columnName)) {
                return true;
            }
        }
        return false;
    ...
    public static Association getAssociationClass(DataModel dataModel, String tableName) {
        Entity entity = dataModel.getEntities().get(tableName);
        /*
        ...
        Association pseudoAssoc = new Association(tableName, leftE, rightE);
        return pseudoAssoc;
    }

    public static End_AssociationClass getAssociationClassEnd(DataModel dataModel, String tableName,
            String columnName) {
        Entity entity = dataModel.getEntities().get(tableName);
        /*
         * Getting the information of ends of the association class
    ...
    public static AuthFunc findAuthFunctionAssociationClass(DataModel dataModel, List<AuthFunc> functions,
            End_AssociationClass end_asc) throws Exception {
        Optional<AuthFunc> authFun;
        if (functions == null || functions.isEmpty()) {
            throw new Exception("There is no auth function created");
        } else {
